...
class TestResultCache(TestCase):
    ...
    def test_cache_base_name_is_necessarily(self):
        ...
        some_cache.cache_db_name = ''
        self.assertRaises(ValueError, lambda: some_cache.cache_db)
    ...
    def test_cache_base_name_accepts_only_str_type(self, wrong_cache_base_type):
        some_cache = ResultCache()
        some_cache.cache_db_name = wrong_cache_base_type
        self.assertRaises(ValueError, lambda: some_cache.cache_db)
    ...
    def test_get_user_result_without_cache_data(self, result_id, user_id):
        self.assertIsNone(self.cache.get_result(result_id, user_id))
    ...
    def test_get_user_result_raise_error_with_invalid_input_data(self, result_id, user_id):
        self.assertRaises(ValueError, lambda: self.cache.get_result(result_id, user_id))

    def test_set_user_result(self):
        for i in range(6):
            self.cache.add_result(i, i)
        for i in range(6):
            self.assertEqual(
                self.cache.get_result(self.cache.get_current_result_id(i), i),
                i,
            )
    ...
    def test_set_user_result_raise_error_with_invalid_input_data(self, data, user_id):
        self.assertRaises(ValueError, lambda: self.cache.add_result(data, user_id))

    def test_delete_user_result(self):
        self.cache.add_result(self.data, 2)
        self.assertTrue(self.cache.delete_result(1, 2))
        self.assertFalse(self.cache.delete_result(55, 4))
    ...
    def test_get_user_current_result_id_without_cache_data(self, user_id):
        self.assertFalse(
            self.cache.get_current_result_id(user_id)
        )
    ...
    def test_set_user_current_result_id(self, new_result_id, user_id):
        try:
            self.cache.set_current_result_id(new_result_id, user_id)
        except ValueError:
            self.assertIsNone(self.cache.get_current_result_id(user_id))
        else:
            self.assertEqual(self.cache.get_current_result_id(user_id), new_result_id)

    def test_increment_current_result_id(self):
        self.cache.set_current_result_id(2, 1)
    ...
    def test_get_results_keynames(self):
        for i in range(5):
            self.cache.add_result(self.data, 1)

        for num in range(1, 5):
            self.assertTrue(f'result:{num}' in self.cache.get_results_keynames(1))

    def test_one_data_but_different_user_id(self):
        self.cache.add_result({'first data': 2}, 1)
        self.cache.add_result({'second data'}, 2)
        self.assertNotEqual(
            self.cache.get_result(1, 1),
            self.cache.get_result(1, 2)
        )
